...
class MenuSqlGenerator extends BaseGenerator
{
    ...
    public function addInstallSql()
    {
        if (!empty($this->addonName)) {
            ...
            $file   = $dir.DIRECTORY_SEPARATOR.'server'.DIRECTORY_SEPARATOR.'addon'.DIRECTORY_SEPARATOR.$this->addonName.DIRECTORY_SEPARATOR.'sql'.DIRECTORY_SEPARATOR.'install.sql';
//            $res    = $this->getMenuSql();
//            $addSql = $res['sql'];
...
//            file_put_contents($file, $sql, FILE_APPEND);
    ...
    public function getRuntimeOutDir()
    {
        if (!empty($this->addonName)) {
            ...
            $dir = $this->outDir . DIRECTORY_SEPARATOR.'server'.DIRECTORY_SEPARATOR.'app'.DIRECTORY_SEPARATOR.'sql'.DIRECTORY_SEPARATOR;
        }
        $this->checkDir($dir);
        if (!empty($this->moduleName)) {
            $this->checkDir($dir);
    ...
    {
        if (!empty($this->addonName)) {
            $dir = $this->rootDir . DIRECTORY_SEPARATOR.'server'.DIRECTORY_SEPARATOR.'addon'.DIRECTORY_SEPARATOR.$this->addonName.DIRECTORY_SEPARATOR.'sql'.DIRECTORY_SEPARATOR;
            $this->checkDir($dir);
            return $dir;
        }else{
            return '';
...
//        return '';
    ...
    public function getMenuSql()
    {
        ...
        $data = [
            ...
            [
                'app_type' => 'site',
                'name' => $table_content.'添加',
                'key' => $key.'_add',
                'parent_key' => $key,
                'type' => $this->table['edit_type'] == 1 ? 2 : 1,
                'icon' => '',
                'api_url' => $this->getRouteName(),
        ...
        $sql = Db::name('permissions')->fetchSql(true)->insertAll($data);
    ...
    public function getRouteName()
    {
        ...
        if ($this->moduleName && ($this->getLowercaseCaseTableName() != $this->moduleName) && $this->className){
            ...
            $route_name = $this->getLowercaseCaseTableName();
        }
