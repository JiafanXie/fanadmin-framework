...
use FanAdmin\src\http\service\admin\addon\cloud\BaseCloudService;
use FanAdmin\src\http\service\admin\addon\cloud\ModuleCloudService;
...
/**
 */
class AddonCloudService extends BaseCloudService
{
    /**
     * 云编译
     * @param $addon
    ...
    public function cloudBuild(string $addon) {
        ...
        dirMkdir($package_dir);

        $compile = (new AddonService())->getAddonConfig($addon)['compile'] ?? [];
        ...
        if (!in_array('wap', $compile)) {
            dirCopy($this->root_path . 'uni-app', $package_dir . 'uni-app', exclude_dirs:['node_modules', 'unpackage', 'dist']);
            $need_build = true;
        }
        // 拷贝admin端文件
        if (!in_array('admin', $compile)) {
            dirCopy($this->root_path . 'admin', $package_dir . 'admin', exclude_dirs:['node_modules', 'dist', '.vscode', '.idea']);
            $need_build = true;
        }
        // 拷贝web端文件
        if (!in_array('web', $compile)) {
            dirCopy($this->root_path . 'web', $package_dir . 'web', exclude_dirs:['node_modules', '.output', '.nuxt']);
            $need_build = true;
        }
        ...
        if ($need_build) {
            ...
            (new AddonDevelopDownloadService(''))->compressToZip($package_dir, $zip_file);
            ...
            (new AddonInstallService($addon))->handleAddonInstall();
        }
        return true;
    }
        ...
        try {
            ...
            if (isset($build_log['data']) && isset($build_log['data'][0]) && is_array($build_log['data'][0])) {
                ...
                if ($last['percent'] == 100 && $last['code'] == 0) {
                    (new AddonInstallService($addon))->installExceptionHandle();
    ...
    public function downloadAddon(string $addon, string $version) {
        $action_token = (new ModuleCloudService())->getActionToken('download', ['data' => ['app_key' => $addon, 'version' => $version]]);
        if (isset($action_token['code']) && $action_token['code'] != 1) {
            if ($action_token['code'] == 401) $action_token = (new ModuleCloudService())->getActionToken('download', ['data' => ['app_key' => $addon, 'version' => $version]]);
    ...
    public function upgradeAddon(array $data = []) {
        $action_token = (new ModuleCloudService())->getActionToken('upgrade', ['data' => $data ]);
        if (isset($action_token['code']) && $action_token['code'] != 1) {
            if ($action_token['code'] == 401) $action_token = (new ModuleCloudService())->getActionToken('upgrade', ['data' => $data ]);
            if ($action_token['code'] != 1) throw new CommonException($action_token['msg']);
        }
